...
3.  Add <code>acts_as_tree</code> to your hierarchical model(s) in <code>app/models/</code>. Nothing else needs to be added to app/models/.
...
5.  Add a database migration to store the hierarchy for your model. By
    ...
    "_hierarchy". Note that you do not need a model file to :
...
7.  If you're migrating away from another system where your model already has a code>parent_id</code> column, run
    ...
    If you're starting from scratch with no prior data you won't need to call <code>rebuild!</code>.
...
Put another thing inside the "physics" node, but this time by appending to the <code>children</code> collection (which is equivalent to <code>.add_child</code>).

  gravity = Tag.create!(:name => 'Gravity')
  physics.children << gravity

Now you should have something that resembles this:

  science.self_and_descendants
